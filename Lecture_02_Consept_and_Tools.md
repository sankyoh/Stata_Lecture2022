...
`count`コマンドは、条件に合致する観察数を返すコマンドです。`count if x==1`では、「1」を返しますが、`count if y==1.1`では、「0」を返しています。`list`コマンドで見たように、y=1.1という観察があることは明らかです。

floatとdoubleは（というかコンピューター全般）、数値を二進数で格納しています。意外なことかも知れませんが、1.1に相当する二進数
